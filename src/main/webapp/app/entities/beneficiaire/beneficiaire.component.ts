...
currentAccount: any;
    ...
    loadAll() {
        this.beneficiaireService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<Beneficiaire[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/beneficiaire'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Beneficiaire) {
        return item.id;
    }
    registerChangeInBeneficiaires() {
        this.eventSubscriber = this.eventManager.subscribe('beneficiaireListModification', (response) => this.loadAll());
    }
